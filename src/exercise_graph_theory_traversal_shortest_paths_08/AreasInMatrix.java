package exercise_graph_theory_traversal_shortest_paths_08;
...
public class AreasInMatrix {
    ...
    private static void dfs(int row, int col, char areaSymbol) {
        ...
        if (inBoubds(row, col + 1) && !visited[row][col + 1] && matrix[row][col + 1] == areaSymbol) {
        ...
        if (inBoubds(row + 1, col) && !visited[row + 1][col] && matrix[row + 1][col] == areaSymbol) {
    ...
    private static boolean inBoubds(int row, int col) {
